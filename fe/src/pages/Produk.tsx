    ...
    <p class="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto animate-fade-in-up delay-100">
      ...
      <div class="container mx-auto px-6 py-12 min-h-[60vh]">
        <Show when={products.loading}>
          <div class="text-center py-20 text-gray-500 animate-pulse">Memuat produk...</div>
        </Show>
        ...
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 animate-fade-in">
            <For each={products()}>
              {(product) => (
                <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 group transform hover:-translate-y-1">
                  ...
                  <h2 class="text-lg font-semibold text-slate-800 mb-1 truncate">{product.name}</h2>
